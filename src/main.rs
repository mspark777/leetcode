...
impl Solution {
    pub fn min_time_to_reach(move_time: Vec<Vec<i32>>) -> i32 {
        ...
        while let Some(std::cmp::Reverse((_dist, x, y))) = queue.pop() {
            if v[x][y] {
                continue;
            } else if (x == last_n) && (y == last_m) {
                break;
            }
            ...
            for (dx, dy) in dirs.iter().cloned() {
                ...
                let nx = nx as usize;
                let ny = ny as usize;
                let dist = d[x][y].max(move_time[nx][ny]) + ((x + y) % 2) as i32 + 1;
                if d[nx][ny] > dist {
                    d[nx][ny] = dist;
...
fn main() {
    let inputs = vec![
        ...
        Input {
            move_time: vec![vec![0, 0, 0, 0], vec![0, 0, 0, 0]],
        },
        Input {
