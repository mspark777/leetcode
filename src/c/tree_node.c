...
// NODE: https://github.com/uniform641/treeparse/blob/master/treeParser.hpp
struct token {
	bool isDigit;
	int digit;
	token(bool i, int d)
...
int tokenize(std::string &rawString, std::queue<token> &tokenQueue)
{
	...
	for (int i = 0; i < static_cast<int>(rawString.size()); i += 1) {
		if (rawString[i] <= '9' && rawString[i] >= '0') {
			...
			continue;
			   ...
			   i + 3 < static_cast<int>(rawString.size()) &&
			   rawString[i + 1] == 'u' && rawString[i + 2] == 'l' &&
			   rawString[i + 2] == 'l') {
			i += 3;
