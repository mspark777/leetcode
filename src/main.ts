class FindLadders {
  private readonly beginWord
  private readonly endWord
  ...
  constructor (beginWord: string, endWord: string, wordList: string[]) {
  ...
  private bfs (): boolean {
    ...
    const queue: string[] = [endWord]
    const visited = new Set<string>([endWord])
    let reached = false
    let distance = 0
  ...
  private dfs (result: string[][], tempPaths: string[], word: string): void {
  ...
  private getNextWord (word: string): string[] {
    const AZ = 'az'
